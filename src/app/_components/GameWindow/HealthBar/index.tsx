...
export default function HealthBar({character}: {character: Player | Enemy}){
    ...
    function handleHpBar(){
        if((character.hp / character.maxhp) < 0.3) return "hsl(358, 100%, 45%)"
    ...
    return (
        <div className={styles.healthbar_wrapper}>
            ...
            <div id={styles.hp} style={{"width":`${(character.hp/character.maxhp)*100}%`, }}></div>
            <div id={styles.maxhp} style={{"backgroundColor":handleHpBar()}}></div>
            {
                character.armour > 0 &&
